...
const App: React.FC = () => {
  ...
  if (!user.isOnboarded) {
    return (
      <div className="min-h-screen bg-[#000B3D] flex flex-col items-center justify-center p-6 space-y-8">
        <div className="animate-fade-up">
          <img
            src="logo.png"
            alt="Brunotech Logo"
            className="w-32 h-32 object-contain mx-auto drop-shadow-[0_0_15px_rgba(127,123,216,0.3)]"
          />
        </div>

        <GlassCard className="w-full max-w-sm space-y-8 py-10 border-white/5">
          <div className="text-center space-y-2">
            ...
            <p className="text-tanzine-100/60 text-sm">
              Design Brunotech • Gestão de Elite
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <Button
              fullWidth
              onClick={() => {
                onboard(formData.description || "Usuário", "BRL");
                addAccount({
  ...
  const handleReset = () => {
    if (
      confirm(
        "ATENÇÃO: Isso apagará TODOS os seus dados permanentemente. Deseja continuar?",
      )
    ) {
      resetStore();
    }
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen relative overflow-x-hidden font-sans bg-[#000B3D] selection:bg-snappy-400 selection:text-white">
      <main className="px-5">
        {activeTab === "Home" && (
          <Dashboard
            onAction={(act) => {
          ...
          <div className="pt-8 space-y-6 pb-24 animate-in fade-in slide-in-from-right-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button className="p-2 bg-white/5 rounded-xl active:bg-white/10">
                  <Search size={20} />
                </button>
                <button className="p-2 bg-white/5 rounded-xl active:bg-white/10">
                  <Filter size={20} />
              ...
              {transactions.length > 0 ? (
                transactions.map((tx) => {
                  ...
                  return (
                    <div
                      key={tx.id}
                      className="flex items-center gap-4 group animate-in fade-in slide-in-from-bottom-2"
                    >
                      <div
          ...
          <div className="pt-8 space-y-6 pb-24 animate-in fade-in slide-in-from-left-4">
            ...
            <div className="space-y-4">
              ...
              <Button
                fullWidth
                variant="ghost"
                className="border-2 border-dashed border-white/5 !bg-transparent text-white/30 hover:border-white/20 transition-all"
                onClick={() => {
        ...
        {activeTab === "Stats" && (
          <div className="pt-8 space-y-6 pb-24 text-center animate-in zoom-in-95 duration-300">
            <h1 className="text-2xl font-display font-extrabold mb-10">
              Configurações
            </h1>

            <GlassCard className="py-8 border-tanzine-500/20 mb-6">
              <PieChart
                size={48}
                className="mx-auto text-tanzine-400 mb-4 opacity-50"
              />
              <p className="font-bold">Análise de Dados</p>
              <p className="text-xs text-white/40 mt-2 px-8 leading-relaxed">
                Adicione mais transações para que a IA possa gerar relatórios
                detalhados sobre seus hábitos.
              </p>
            </GlassCard>

            <div className="pt-10 border-t border-white/5">
              <h3 className="text-sm font-black text-red-500 uppercase tracking-[0.2em] mb-4">
                Zona de Perigo
              </h3>
              <button
                onClick={handleReset}
                className="flex items-center justify-center gap-2 w-full p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 font-bold active:scale-95 transition-all"
              >
                <Trash2 size={18} />
                Zerar Todos os Dados
              </button>
              <p className="text-[10px] text-white/20 mt-3 px-6 italic">
                Esta ação é irreversível e apagará todo o histórico de
                transações e contas.
              </p>
            </div>
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto h-20 bg-[#000620]/90 backdrop-blur-2xl border-t border-white/5 px-8 flex items-center justify-between safe-bottom z-50">
        {NAV_ITEMS.map((item) => (
          <button
            key={item.label}
          ...
          >
            <div
              className={`p-1 rounded-lg transition-colors ${activeTab === item.label ? "bg-snappy-400/10" : ""}`}
            >
              {item.icon}
            </div>
          ...
          <GlassCard className="w-full max-w-sm mb-4 relative z-10 animate-in slide-in-from-bottom-full duration-500 shadow-2xl shadow-black/50 border-tanzine-500/30">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-tanzine-200 uppercase tracking-widest ml-1">
                  Categoria
                </label>
